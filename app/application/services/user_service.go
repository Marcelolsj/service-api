...
func (this *UserService) CreateUser(ctx context.Context, createUserDto dtos_users.CreateUserRequestDTO) (*dtos_users.CreateUserResponseDTO, error) {
	user := mappers.MapCreateUserDTOToUser(createUserDto)

	err := this.UserUseCase.CreateUser(ctx, user)
	if err != nil {
		return nil, err
	}
